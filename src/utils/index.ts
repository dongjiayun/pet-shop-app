...
export function getDictLabelByVal(
    data: Dicts,
    val: number | string | undefined,
    props: dictProp = { value: "value", label: "label" }
): string {
    let result = "";
    if (val || val === 0) {
...
export function getDictNameByVal(
    data: Array<object>,
    val: number | string,
    props: dictProp = { value: "value", label: "name" }
): string {
    let result = "";
    if (val || val === 0) {
        if (data && data.length > 0) {
...
export function toThousandsNum(value: number, precision = 2): number | string {
    ...
    return round(Number(value), precision)
        .toFixed(precision)
...
export const numberUnitFormat = (
    ...
    if (num < 10000) {
        return isInteger
            ? toThousandsNum(round(num, 0), 0)
            : toThousandsNum(round(num, precision), precision);
...
export const getDecimals = (num: number | string): string => {
    ...
    return numstr.length > 1
        ...
        : numstr.length === 1
          ? "0" + numstr.substring(numstr.length - 2, numstr.length)
          : "00";
};
